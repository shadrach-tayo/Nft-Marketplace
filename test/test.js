...
describe("NFTMarket", function () {
  it("Should interact with the token contract", async function () {
    ...
    await market
      .connect(userAddress2)
      .createMarketItem(nftContractAddress, 9, 1000, {
        value: ethers.utils.parseEther("0.01"),
      });
  ...
  it("should lock nft and unlock it", async () => {
    const [owner, userAddress, userAddress2, userAddress3] =
      await ethers.getSigners();
    ...
    await nft.connect(userAddress).createToken('a');

    await market
      .connect(userAddress)
      .createMarketItem(nftContractAddress, 1, 1000, {
        value: ethers.utils.parseEther("0.01"),
      });

    const item1 = await market.fetchMarketItem(1);
    expect(item1.seller === userAddress);
    console.log('item1 ', item1);

    await market.connect(userAddress).lockNft(1);
     const item2 = await market.fetchMarketItem(1);
     expect(item2.seller === userAddress);
     expect(item2.locked === true);
